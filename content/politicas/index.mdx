...
Esta versão corrige a fundamentação jurídica da versão 0.5 e reorganiza o item 19. Fora dessa reorganização, **nenhuma regra de conduta foi flexibilizada**: o que mudou foi a atribuição correta de cada regra à sua origem — lei, orientação regulatória, referência internacional ou decisão da empresa.
...
| 19 | **Reorganizado.** A lista única de requisitos mínimos da v0.5 passa a ter dois níveis: requisitos de base do item 19.3, exigíveis de todo sistema (repositório `elinsa-do-brasil`, hospedagem mantida pela empresa, ausência de segredos no código, documentação de uso e verificação de segurança manual ou automatizada), e requisitos adicionais do item 19.4, exigíveis quando o sistema tratar informação confidencial ou restrita (revisão por terceiro, dois mantenedores e sucessão, dependências e licenças, autenticação no diretório). O item 19.5 passa a admitir a revisão de segurança como alternativa à compreensão integral do código. Os itens 19.6 a 19.8 são novos ou ampliados: permanência em uso durante a avaliação do Comitê, desativação como última alternativa e dever de a TI apoiar a área usuária no cumprimento dos requisitos |
